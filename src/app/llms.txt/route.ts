...
export async function GET() {
  ...
  const body = [`# MessageLab`, ``, hero, ``, `## How it works`, ...steps.map((step, index) => `${index + 1}. ${step}`), ``, `## Public URLs`, ...["/", "/about", "/blog", "/contact", "/help", "/docs", "/download-apk", "/status", "/support", "/privacy", "/terms", "/anti-spam"].map((path) => `- ${path}`), ``, `## Plans`, ...plans.map((plan) => `- ${plan.displayName}: ${plan.priceMonthly} ${plan.currency}/month`), ``, `Contact: ${content.footer.footer_support_email?.body || "support@messagelab.tech"}`].join("\n");
